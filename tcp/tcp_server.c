...
int tcpSocket(void)
{
    ...
    sockFd = socket(AF_INET6, SOCK_STREAM, IPPROTO_IP);
    if (sockFd == -1) {
        lErr("socket() failed...");
    }
    else {
...
int tcpServer(int port)
{
    ...
    sock = tcpListenSocket(port);
    if (sock == -1) {
        lWrn("tcpListenSocket() failed");
        isRun = 0;
    }
